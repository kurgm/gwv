...
def indexOfFirstKanjiBuhinLine(sname: list[str], kage: KageData):
    ...
    for i, sname_i in enumerate(sname):
        ...
        for line in kage.lines:
            if line.stroke_type == 99 and line.part_name.startswith(firstKanji):
                return line
        return None
    return None
...
class IdsValidator(Validator):
    ...
    def is_invalid(self, ctx: ValidatorContext):
        ...
        ) or (sname[1] in ("u2ff1", "u2ff3") and sname[0] in ("u2ff0", "u2ff2"))
        ...
        if sname[0] in ("u2ff0", "u2ff2"):
            # [-01] + [-02] or [-01] + [-01] + [-02]
            if firstBuhinType in ("03", "04", "09", "14", "24") and x2 - x1 > 175.0:
                # 左右のIDSだが最初が上下の部品
            ...
            if (
                not isComplicated
                and firstBuhinType not in ("01", "08")
                and aspect > 1.8
            ):
                # 左右のIDSだが最初の部品が横長の配置
            ...
            # [-03] + [-04] or [-03] + [-03] + [-04]
            if firstBuhinType in ("01", "02", "08") and y2 - y1 > 175.0:
                # 上下のIDSだが最初が左右の部品
            ...
            if (
                not isComplicated
                and firstBuhinType not in ("03", "09")
                and aspect < 0.65
            ):
                # 上下のIDSだが最初の部品が縦長の配置
